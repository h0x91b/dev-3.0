...
- **Codex variant** — a short pointer `SKILL.md` (shared with the generic dirs, see below) plus a hook-aware `PROTOCOL.md` carrying the full body with the `/bin/bash` shell note. The body itself reaches dev3-launched Codex out-of-band as a developer message via `-c developer_instructions=...`, including scratch tasks and resume (decision 115), so the pointer's description tells it not to invoke the skill; `PROTOCOL.md` is the fallback for sessions started outside the dev3 launcher. Codex lists both `~/.codex/skills` and `~/.agents/skills`, which is why the generic copy is a pointer too — see `decisions/2026/09/25/codex-and-generic-dev3-skills-become-pointers.md`
...
- **Generic variant** — the same short pointer `SKILL.md`, MANDATORY to invoke for an agent that does not already hold the protocol; the full body sits in `PROTOCOL.md` next to it (for Gemini it is the only protocol channel, reached through that pointer) with full manual status management instructions ("CRITICAL — NON-NEGOTIABLE"), requiring agents to run `dev3 task move` at start/end of every turn
