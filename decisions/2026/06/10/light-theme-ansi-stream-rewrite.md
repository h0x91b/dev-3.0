...
Claude Code 2.1.112's `/theme` preview exposed a narrower cross-polarity case that generic luminance gating cannot solve: its non-ANSI light themes emit fixed red/green diff backgrounds (`255,220,220` / `220,255,220`) and the colorblind variant emits blue (`219,237,255`), while the dark variants emit mirrored fixed backgrounds. When the selected Claude theme disagrees with dev3, the filter now remaps only those observed base/word-highlight pairs to their opposite-polarity counterparts and applies a stronger foreground target on the remapped row. Exact matching is deliberate: generic background inversion would corrupt vim, syntax themes, and application-owned highlights.

Dim hurt the dark theme too, not just light. ghostty renders SGR `2` as 50% alpha; Claude Code's select-prompt (`AskUserQuestion`/plan) draws option descriptions, numbers, separators and hints with Ink `dimColor` (verified in `cli.js`), and the input ghost/placeholder suggestion as `\x1b[0;2m<text>\x1b[0m`. At 50% alpha the default dark fg (`#c0caf5`) collapses to a low-contrast blue-gray — unreadable. A first pass *dropped* dim in both modes; that made the placeholder render at full intensity, indistinguishable from typed input. The filter now *emulates* dim over the default fg as an explicit muted gray (dark `rgb(112,120,150)`, light `rgb(130,130,130)`): readable yet visibly secondary. The dim is resolved at sequence end (`dimPending`) and only emulated when no explicit fg accompanies it (an explicit color wins and dim is dropped, as before); `fgExplicit` tracks this. The emulated gray is reset by SGR 0 and by SGR 22 (dim off → also emit `39`), tracked by `dimActive`.

## Risks

Emulating dim flattens the muted-vs-normal *brightness* gradient slightly (one fixed gray instead of a per-color blend), and colored dim still loses its dim (the color shows full) — acceptable; the dominant case is dim over default fg. Colors written while one theme is active stay resolved in scrollback after a theme switch until the app repaints. Backgrounds remain untouched except for the exact observed Codex/Claude diff palette pairs; this avoids inverting unrelated intent but means upstream palette changes need an explicit update. The gate only sees SGR; an app that sets a background via OSC or DEC private modes would not trip it (not observed in practice).
...
- Inverting every opposite-polarity truecolor background: broader, but breaks application-owned selections and editor color schemes; exact known diff pairs are safer.
