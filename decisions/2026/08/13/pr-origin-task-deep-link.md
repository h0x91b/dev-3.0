...
- **A settings toggle.** Deferred here, then **un-deferred by issue #1340** — a public
  PR would otherwise advertise dev3 usage. Now a default-on
  `GlobalSettings.prOriginTaskLink`; see
  [pr-deep-link-opt-out-and-block-ordering](pr-deep-link-opt-out-and-block-ordering.md)
  and [pr-deep-link-single-line-handoff](../14/pr-deep-link-single-line-handoff.md).
