...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="Output directory. Defaults to a temp dir OUTSIDE the repo so the "
        "inventory is never accidentally committed. The inventory is a "
        "throwaway review aid, not a manifest artifact.",
    )
    ...
    if args.out:
        ...
        out_dir = Path(tempfile.gettempdir()) / "ux-inventory"
    ...
    out_file = out_dir / "ux-inventory.json"
    out_file.write_text(json.dumps(inventory, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Wrote {out_file}")
    print("This is a throwaway review aid — do NOT commit it into the repo.")
    return 0
