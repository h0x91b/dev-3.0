...
- **Artifact comment mode shares this review — `Observed` (2026-09-16):** the artifact viewer's one header toggle turns the frame into a picking surface (click reports selector, text, nearest heading) and opens a 22rem aside with the *same* composer and bubbles (`src/mainview/review`). The frame draws the numbered pins itself; ids it cannot place after a republish show as `Not found in this version`, never dropped. One batch button, unsent only. Escape leaves comment mode first. The image viewer has the same toggle (drag a region → `image-region`), the file preview offers `Add comment` on a text selection (`file-range`), and a terminal selection shows a `Comment` chip for six seconds (`terminal-text`) — all four open the same composer and land in the same review.
