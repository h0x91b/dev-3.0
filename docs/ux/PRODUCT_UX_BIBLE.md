...
| Sidebar | Active-task jump list (readiness-tier work queue: NEEDS YOU → custom columns → WAITING, priority-sorted, visible `P{n}` badge) | destination, task jump, priority re-order (badge picker), terminal preview, search + token-DSL task filter (funnel, active-statuses pool) | durable config | `ActiveTasksSidebar.tsx`, `sidebarTiers.ts`, `FilterFunnel.tsx` |
