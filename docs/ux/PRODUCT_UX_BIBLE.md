...
**An agent output is mounted by App; a pane only offers it a slot (`Observed`, 2026-09-07).** Images always use the App-hosted lightbox. One artifact viewer has two presentations: the **docked drag-resizable right-hand panel by default**, and that lightbox when `openArtifactsInPopup` is on or nothing publishes a slot (archived modal, immersive fullscreen, narrow, an off-screen task's toast). It portals into the slot rather than being owned by a screen — one viewer, re-hosted on a flip (`decisions/2026/09/07/artifact-panel-default-popup-opt-in.md`). A new task-scoped viewer inherits the lightbox; a second dock needs its own record.
