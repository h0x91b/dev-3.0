...
A keyboard-summoned palette with **two modes on one shared shell** (`PaletteShell`). **Cmd+K** = navigation: a **"go to project or task"** palette (`GoToPaletteModal`) with one active **mode** at a time, cycled by tapping **Shift** (a lone tap, not hold) or clicking the footer mode switcher: **Projects** (fuzzy-jump, ⌘1..9 badges) → **Both** (projects *and* tasks interleaved in one recency timeline) → **Tasks** (active tasks across every project). Rows in the two grouped modes bucket by **recency** into `Today` / `Yesterday` / `This week` / `Older` (empty buckets render no header) — a task's recency is the later of its `updatedAt` and its **last-opened time** (so a task you engaged with today ranks as Today even if its record wasn't modified — `updatedAt` alone misses terminal work), a project's is its **last board-view time** — *displaying the project's board* counts, but opening a task inside it does **not** (the task is already its own row) — so in **Both** a project whose board you just viewed interleaves *among* the recent tasks rather than sitting in a trailing section. Task rows show a round status dot + project badge; project rows a square marker + ⌘ badge. Opens in the last-used mode (default **Projects**). Enter navigates (complements `Cmd+1..9` and the breadcrumb dropdown). **Cmd+Shift+P** = actions: fuzzy-match a command label, Enter runs it via `handleMenuAction` — a DOM mirror of the native menu, not a second command runner; only context-applicable commands show. Both fuzzy via `utils/fuzzyMatch.ts`. Destructive (delete/cancel/complete) and modal/inline flows (rename, overview, note, spawn, duplicate) are excluded from the quick palette by policy. See Surface model below.
...
| Command palette (Cmd+K nav / Cmd+Shift+P actions) | Type-to-find nav + type-to-run commands (two modes, one shell) | destination, fuzzy search, object jump (project **and** task), command runner (action mode, via handleMenuAction) | destructive action, modal/inline flow, durable config without friction, dense filters | `PaletteShell.tsx`, `GoToPaletteModal.tsx`, `CommandPaletteModal.tsx`, `commands.ts` |
...
The **command palette** is keyboard-only by design (no toolbar/breadcrumb button → sidesteps button-creep) and runs in two sibling modes on one shared `PaletteShell`. **Navigation (Cmd+K):** a `GoToPaletteModal` with one active **mode** (Projects · Both · Tasks — where Both interleaves projects and tasks in one recency timeline) cycled by a lone Shift tap or the footer mode switcher; grouped modes bucket rows by recency (task `updatedAt` / project last-board-view) into Today / Yesterday / This week / Older. The matcher (`utils/fuzzyMatch.ts`) is the single matcher for short UI entities. **Actions (Cmd+Shift+P):** fuzzy-runs a command via `handleMenuAction` (DOM mirror of the native menu), listing only context-applicable commands and excluding destructive + modal/inline flows by policy. Cmd+K and the **Option+Tab task switcher** are complementary, not redundant: Option+Tab *hold-cycles* the active tasks (fast alt-tab toggle, MRU order), Cmd+K *type-searches* them (jump to a named task). Hotkeys avoid `Cmd+T` (universal new-tab; the live terminal underneath intercepts it). The navigation-vs-action question is resolved as **two-surfaces-one-shell** — see `UX_DECISIONS.md` (2026-06-18, 2026-07-06) and decision record 072.
