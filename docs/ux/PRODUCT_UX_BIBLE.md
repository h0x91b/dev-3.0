...
| project grouping (`Space`) | the **dashboard** only: a left rail listing `All projects` + each space + the computed `Home` (each space/Home row carrying its needs-you/working split), a collapsible group header per space, membership chips + a `Spaces…` action on the project row, and rename/delete behind one `…` on the header. Selecting a rail entry **filters the overview in place**. The rail is also the **one** surface that reorders spaces (see the row below) | a space route/screen, a breadcrumb into a space, a merged space board, a space colour, a stored `Home`, membership changed by drag, a per-space entry in global nav, a space-order control anywhere but the rail | a space is a grouping, not a place — the moment it gains a route the next question is "where is this place's board", which is the decision #257 rejected. Filtering keeps one board per project and one screen. Drag stays order-only because with many-to-many memberships a drag cannot show which edge it removed. Zero spaces ⇒ the dashboard renders its pre-Spaces flat list, rail absent |
| space **order** | the **rail**, two paths over one write (`reorderSpaces`): drag a row for pointers, and a **reorder mode** (toggle beside the `Spaces` label → rows lose their filter role and gain ↑/↓, `Home` leaves the list) for touch and keyboard. A resting grip glyph on each row is what advertises the drag | a second grip on the dashboard's space group headers, per-row ↑/↓ as always-visible controls, a `Manage spaces` overlay | the rail already **is** the collapsed name-only list a reorder needs — a grip on a content header meant dragging across a page hundreds of pixels tall, and it never existed on touch (`hidden md:inline-flex` + HTML5 drag). Per-row arrows were measured and rejected: 224px minus padding leaves 180px of row, and grip + two arrows eats ~74px, starving the name the row exists to show. Mode-scoping the arrows buys the width back by dropping the counts, which say nothing about position |
