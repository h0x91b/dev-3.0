...
| project grouping (`Space`) | the **dashboard** only: a left rail listing `All projects` + each space + the computed `Home` (each space/Home row carrying its needs-you/working split), a collapsible group header per space, membership chips + a `Spaces…` action on the project row, and **every space action behind one `…`** — carried identically by the rail row and the group header, placed against the space name rather than the far side of the column: `Edit projects…`, `Move up`/`Move down`, `Rename`, `Delete`. Selecting a rail entry **filters the overview in place**. The rail owns `New space`, as the last entry of its space list (see the row below for order), and appears only while the dashboard's own column is ≥1024px — measured on the container, never the viewport | a space route/screen, a breadcrumb into a space, a merged space board, a space colour, a stored `Home`, membership changed by drag, a per-space entry in global nav, a reorder mode in the rail, an add-only `+` beside the space name, a rail that hides on a viewport media query | a space is a grouping, not a place — the moment it gains a route the next question is "where is this place's board", which is the decision #257 rejected. Filtering keeps one board per project and one screen. Drag stays order-only because with many-to-many memberships a drag cannot show which edge it removed. One menu, not a menu plus a `+`: an add-only control cannot answer "take this project out", which sent the user to the project's own row to undo something they did from the space. The rail carries the same menu because a filter you cannot rename from is only half a surface. The rail's own gate is its **container's** width: it shares the window with app chrome in the desktop shell and with nothing at all in a browser tab, so the viewport was measuring the wrong box (`useContainerWidth`; width 0 means *not measured*, never *narrow*). Zero spaces ⇒ the dashboard renders its pre-Spaces flat list, rail absent |
