...
| HTML Artifact | — (conditional Runtime-bar entry) | docked/fullscreen task workspace | task | view, resize, fullscreen, navigate history, find in content (⌘F), download HTML/ZIP | `types.ts (SharedArtifact)`, `TaskArtifactViewer.tsx`, `show-artifact.ts` |
