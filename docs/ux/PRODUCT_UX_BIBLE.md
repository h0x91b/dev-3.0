...
Walked on an empty `DEV3_HOME`: the flow **completes**, and a new install is **never at zero
projects** (first boot creates the Operations board), so the dashboard's zero-projects empty state
is unreachable code. First run explains; it does not rescue. Why + evidence:
[first-run-advertises-help-mode](../../decisions/2026/08/22/first-run-advertises-help-mode.md).
...
- Multi-step tours are no longer banned as a class (lifted above).
