...
**Invitation:** after 3 hours of active use, then 1, 3, 9 and every 14 days from postponement; manually enabling starts a recurring 14-day schedule. Timing persists across restarts, and the host prevents duplicate prompts across windows. Active = focused/visible with trusted key/pointer/wheel/touch input within 60s; 15s heartbeats union windows, gaps >30s earn nothing. Calm = dashboard/project without active task, ≥10s since input, no typing context, back layers, portal overlays, artifact, help, hint or tour. One React invitation shows 2–3 benefits and a small static example; “Use Full interface” switches immediately, “Remind me later”/dismiss postpones. No forced switch or second confirmation. This delayed consent invitation is the narrow exception to §5.4a's no-centre-teaching rule. Acceptance stops reminders and shows one dismissible lesson on hiding individual controls and restoring Simplified Mode. Returning switches immediately; optional feedback must not block it.
