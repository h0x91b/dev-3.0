...
| Task image viewer | Task-bound lightbox for images/clips from `dev3 show-image`/`show-video` (history rail, newest first) | image display, clip play (no autoplay), history nav (thumbnails + prev/next + arrows), copy image, download image (header icon + own right-click menu), reveal path, clear (destructive) | global destination, task lifecycle/git mutation, persistent inspector button (badge is conditional), SVG render (v1) | `TaskImageViewer.tsx` |
...
| Context menu | Right-click / long-press object actions; ONE menu per object class, shared by every surface | object action, open-in, destructive | global destination | `TaskContextMenu.tsx` |
...
**GitHub PR review layer (read-only) — `Observed` (2026-07-19):** when the task has a PR, the viewer renders its review threads inline on their lines (same widget/extend mechanism as local comments; GitHub glyph, author, timestamp, markdown body) plus a collapsible **"Conversation (N)"** strip at the top of the diff stream for top-level PR comments. That strip is the layer's one control cluster (collapse, show-resolved toggle, refresh, fetched-at, PR link) — never toolbar buttons. Unresolved threads expand by default; resolved hide behind the toggle; threads that no longer map onto the diff collapse into a per-file "Outdated" group (absent files group inside the conversation block) — nothing is silently dropped. Inline anchoring is **branch-mode only**; other modes show a slim "N review threads → Branch diff" hint. Per-thread actions: `Send to agent` (`secondary` — a fix prompt into the task terminal; the surface primary stays Copy review), `Open on GitHub` (`link`), `Include in export` (opt-in; exported entries carry an origin marker). **Hard rule: no writes to GitHub from this surface — links out instead.** Data: one cached on-demand RPC (gh GraphQL) on diff open / manual refresh; badge counts stay on the background PR poller.
...
- **Artifact comment mode shares this review — `Observed` (2026-09-16):** the artifact viewer's one header toggle turns the frame into a picking surface (click reports selector, text, nearest heading) and opens a 22rem aside with the *same* composer and bubbles (`src/mainview/review`). The frame draws the numbered pins itself; ids it cannot place after a republish show as `Not found in this version`, never dropped. One batch button, unsent only. Escape leaves comment mode first. The image viewer has the same toggle (drag a region → `image-region`), and the file preview offers `Add comment` on a text selection (`file-range`) — all three open the same composer and land in the same review. A terminal selection only copies (its `Comment` chip was cut 2026-09-24 as intrusive); stored `terminal-text` comments still render.
...
### 5.10 Simplified Mode onboarding + personal hiding — `Observed`

**Simplified Mode** (Settings → Appearance) is temporary onboarding toward the **Full interface**. Fresh installations start simplified; upgrades preserve the existing choice. Mode-owned preset hiding is separate from personal `hiddenControls`; switching either way preserves personal choices. New controls outside the closed preset remain visible. Configuration stays in Settings, never a header toggle or checklist page.

**Invitation:** after 3 hours of active use, then 1, 3, 9 and every 14 days from postponement; manually enabling starts a recurring 14-day schedule. Timing persists across restarts, and the host prevents duplicate prompts across windows. Active = focused/visible with trusted key/pointer/wheel/touch input within 60s; 15s heartbeats union windows, gaps >30s earn nothing. Calm = dashboard/project without active task, ≥10s since input, no typing context, back layers, portal overlays, artifact, help, hint or tour. One React invitation shows 2–3 benefits and a small static example; “Explore the full interface” switches immediately, “Remind me later”/dismiss postpones. No forced switch or second confirmation. This delayed consent invitation is the narrow exception to §5.4a's no-centre-teaching rule. Acceptance stops reminders and shows one dismissible lesson on hiding individual controls and restoring Simplified Mode. The invitation and lesson temporarily suppress the header help callout. Returning switches immediately; optional feedback must not block it.
...
**Restoring — task info panel:** one conditional restore control in panel chrome beside ⚙/collapse (§5.1; no bar-budget cost). Dropdown groups by bar: click runs the action, return icon restores (hover on desktop, always visible on touch); permanent "Show all". Ids: Context bar (`diff-include-tests`), Session/Agent bar (`spawn-agent`, `bug-hunters`, `hibernate`, `scheduled-message`, `tmux-pane-controls`), Runtime bar (`scripts-runner`, `setup-dev-server`).
...
**Non-toolbar surfaces** — no right-click affordance of their own, hidden by the Simplified Mode preset: `stats-nav`, `automations-tab`, `keyboard-shortcut-editor` (the ⌘/ reference overlay is a different surface and stays), `custom-column-creation`, `label-creation`, `spawn-variant`.
...
Evidence: `hideable-controls.ts`, `hidden-controls.ts`, `global-settings-cache.ts`, `HideableControl.tsx`, `PanelRestoreControl.tsx`, `InterfaceOnboarding.tsx`, `useInterfaceOnboarding.ts`, yaml `surfaces.settings.simplify_mode`.
