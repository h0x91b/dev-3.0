...
- **The inline review is durable, not clipboard-only.** Comments persist per task (`localStorage`) and survive unmount / diff reload / app restart; the clipboard is a *transport*, not the store. The only ways to clear the review are the explicit **Reset review** button and (optionally) a successful copy. Leaving the surface does **not** discard it — so no "discard review?" guard on close.
