...
- **Rule:** The Cmd/Ctrl+K navigation palette (`GoToPaletteModal`, was `ProjectQuickSwitchModal`) lists fuzzy-searchable sections on the shared `PaletteShell`: **Tasks first** — every active task across all projects sorted by `updatedAt` desc and **bucketed by date into Today / Yesterday / This week / Older** (empty buckets render no header), each with a status dot + project badge — then **Projects** (⌘N badges). Tasks-first + date buckets means Cmd+K → Enter lands on the most-recent task and today's work is scannable at a glance. No new shortcut. (Session MRU is intentionally *not* used here — cross-bucket bubbling would fight the date grouping; it stays the ordering for the Option+Tab hold-cycle switcher.)
