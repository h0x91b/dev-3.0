...
## 2026-07-06 — Cmd+K "go to project or task" with a mode toggle (Projects · Both · Tasks)

- **Rule:** The Cmd/Ctrl+K palette (`GoToPaletteModal`, was `ProjectQuickSwitchModal`) has ONE active **mode**, cycled by a lone **Shift tap** (not hold; Shift+key still types capitals) or by clicking a footer **mode switcher**: **Projects** (fuzzy-jump, ⌘1..9 badges) → **Both** (projects AND tasks interleaved in one recency timeline) → **Tasks** (all projects' active tasks). Grouped modes bucket rows by **recency** into Today / Yesterday / This week / Older — a task's recency is `max(updatedAt, last-opened)` (`recordTaskVisit` stamps last-opened on navigating to a task, so terminal work that never bumps `updatedAt` still ranks it Today), a project's is its **last board-view time** (`recordProjectBoardView`, stamped only when the project's board is displayed — opening a task inside it does NOT count, since the task is already its own row), so a project whose board you just viewed interleaves among recent tasks in **Both** (not a trailing Projects section). Opens in the **last-used mode** (localStorage; default **Projects**). No new shortcut key.
- **Why:** A single unified list mixed projects and tasks; explicit modes keep each view focused and gave the owner (who mostly task-switches) a durable task view via sticky last-mode, without a second command/shortcut (avoids shortcut-creep, the #1 anti-pattern). Shift-*tap* (vs the Option+Tab switcher's Shift-*hold* scope) was chosen so a quick toggle needs no chord; the clickable switcher makes it discoverable + touch/AT-reachable. Rejected: a dedicated Cmd+Shift+K task palette (second surface); Shift-hold (fights typing + no persistence).
- **Status:** Implemented (browser-verified). Evidence: `GoToPaletteModal.tsx`, `PaletteShell.tsx` (footerLeft + Shift-tap + resetKey), `App.tsx` (goToTasks/goToProjectTasks), bible §4.
