...
## 2026-07-06 — Cmd+K "go to project or task" with a mode toggle (Projects · This project · All tasks)

- **Rule:** The Cmd/Ctrl+K palette (`GoToPaletteModal`, was `ProjectQuickSwitchModal`) has ONE active **mode**, cycled by a lone **Shift tap** (not hold; Shift+key still types capitals) or by clicking a footer **mode switcher**: **Projects** (fuzzy-jump, ⌘1..9 badges) → **This project** (current project's active tasks) → **All tasks** (all projects' active tasks, with a project badge). Task modes bucket rows by `updatedAt` into Today / Yesterday / This week / Older (empty buckets render no header). Opens in the **last-used mode** (localStorage; default **Projects**); **This project** is hidden when no project is in view. No new shortcut key.
