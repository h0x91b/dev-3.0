...
- **Rule:** The Cmd/Ctrl+K navigation palette (`GoToPaletteModal`, was `ProjectQuickSwitchModal`) lists two fuzzy-searchable sections on the shared `PaletteShell`: **Tasks first** — every active task across all projects ordered by recency (session-visited first via `state.taskMru`, then the rest by `updatedAt` desc so tasks worked on today lead even when the MRU is empty, e.g. a fresh browser tab), each with a status dot + project badge — then **Projects** (⌘N badges). Tasks-first means Cmd+K → Enter lands on the most-recent task. No new shortcut.
