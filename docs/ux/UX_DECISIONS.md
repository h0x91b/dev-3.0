...
- **Rule:** The Cmd/Ctrl+K palette (`GoToPaletteModal`, was `ProjectQuickSwitchModal`) has ONE active **mode**, cycled by a lone **Shift tap** (not hold; Shift+key still types capitals) or by clicking a footer **mode switcher**: **Projects** (fuzzy-jump, ⌘1..9 badges) → **Both** (projects AND tasks interlaced — task date-buckets then a Projects section) → **Tasks** (all projects' active tasks, with a project badge). Task rows bucket by `updatedAt` into Today / Yesterday / This week / Older (empty buckets render no header). Opens in the **last-used mode** (localStorage; default **Projects**). No new shortcut key.
