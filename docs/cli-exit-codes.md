...
| `19` | `CLI_EXIT_CODE_EVENT_CURSOR_INVALID` | `dev3 events --from <cursor>` was given a cursor it cannot parse, and **nothing** was read. A cursor is a position, not a duration: `<iso>.<8-hex>` exactly as printed by a previous run, or a bare ISO instant for a wider sweep. Distinct from code `3` so a caller that lost its cursor can recognise this one case and fall back to a bare `dev3 events` instead of treating it as a misspelled command. The cursor is never silently degraded into a time window — one that quietly becomes "the last day" skips everything older and reports success. |
