...
1. **Signing & distribution**: initial signing identity is ittaiz's personal Apple account (decided in PR #969 review). Practical consequence: **TestFlight requires a paid Apple Developer Program membership** — with a free Apple ID, distribution is direct Xcode install to the developer's own devices with 7-day provisioning renewal (fine for the build/dogfood phase); enrolling in the paid program upgrades the path to TestFlight without any code change. App Store later — the app is a client to the user's own server, same category as SSH clients, low review risk.
