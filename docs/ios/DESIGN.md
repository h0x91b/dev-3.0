...
- **Multi-server**: first-class list of paired Macs; one active at a time; switcher on the entry screen.
...
1. **Distribution**: default **TestFlight** (personal team) first; App Store later — the app is a client to the user's own server, same category as SSH clients, low review risk.
2. **Bundle id / name**: placeholder `com.h0x91b.dev3.mobile`, display name "dev3". Needs the user's Apple developer account details at T4.4.
