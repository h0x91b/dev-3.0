...
The native URLSession RPC client applies a bounded 192 MiB receive policy before resuming each `/rpc` socket. This is a client-side safety bound chosen to accommodate current large Diff responses and media wire forms, including Base64 artifact downloads; it is not a universal server response maximum. The `/pty` client keeps a separate 1 MiB per-message ceiling because terminal output is server-batched. Frames above either endpoint's native bound fail explicitly and are never silently truncated.
