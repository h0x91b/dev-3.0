...
| TestFlight workflow | Account boundary | Unsigned Release validation and metadata checks pass, including the embedded UserDefaults privacy declaration for reason CA92.1. Signed archive/export tooling rejects a missing privacy manifest or mismatched team, application ID, or default Keychain group; the first real archive and upload require the owner's Apple account configuration. |
