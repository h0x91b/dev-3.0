...
**Link the PR back to this task.** Unless the user turned it off in dev3 settings, whenever you open a pull request for this task (\`gh pr create\`) end the description with a deep link back to the task so reviewers can jump into it. Take \`<TASK_ID>\` from \`dev3 current\`, and after the existing description add a blank line, then a \`---\` line on its own, then this exact line (GitHub strips \`dev3://\` links, so the \`https\` form is the clickable one and the raw link is a copy-paste fallback):
