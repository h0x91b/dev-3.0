 ...
 *
 * Terminal/rpc/boot stay ON in the app (that is what they were written for) and OFF under vitest,
 ...
 * The expensive `freeze` channel is opt-in even outside tests.
 */

export type DebugChannel = "terminal" | "rpc" | "boot" | "freeze";
...
export function debugEnabled(channel: DebugChannel): boolean {
	...
	if (override === null) return channel !== "freeze" && !underTest();
