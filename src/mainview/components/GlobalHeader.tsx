	...
	return (
		...
		<div className="relative z-30 flex items-center justify-between px-5 py-2.5 border-b border-edge flex-shrink-0 glass-header">
			...
			<div className="flex items-center gap-1.5 flex-shrink-0">
				...
				{/* TEMP: toast preview button - remove after review. Fires one toast of each variant. */}
				<button
					onClick={() => {
						toast.error("Error toast: something failed (auto-dismiss in 30s)");
						toast.success("Success toast: operation completed");
						toast.info("Info toast: just so you know");
						toast.warning("Warning toast: heads up");
					}}
					className="flex items-center gap-1 text-warning hover:text-warning transition-colors px-2 py-1 rounded-lg hover:bg-elevated"
					title="TEMP: preview toasts"
				>
					<span className="text-[1.125rem] leading-none" style={{ fontFamily: "'JetBrainsMono Nerd Font Mono'" }}>{"\uf0f3"}</span>
					<span className="text-[0.6875rem] font-medium">Toast</span>
				</button>
