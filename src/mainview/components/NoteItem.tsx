	...
	return (
		<div className="mb-2 rounded-lg bg-base border border-edge p-2 group">
			<div className="flex items-center justify-between mb-1">
				<span className={`text-dense font-medium px-1.5 py-0.5 rounded ${
					isAi ? "bg-accent/10 text-accent" : "bg-raised text-fg-3"
				}`}>
					{isAi ? t("notes.sourceAi") : t("notes.sourceUser")}
				</span>
				<div className="flex items-center gap-1.5">
					<span className="text-dense text-fg-muted">{formatDate(note.updatedAt)}</span>
					<button
						onClick={onDelete}
			...
			{isAi ? (
				...
				>
					...
					/>
					{isPasting && (
						<span className="text-dense text-accent animate-pulse">{t(pasteKind === "text" ? "paste.savingText" : "images.pasting")}</span>
					)}
