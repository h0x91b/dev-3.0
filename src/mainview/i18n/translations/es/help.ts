const help = {
	...
	"help.board.filterBar.body":
		"Una sola caja busca y filtra el tablero. Escribe texto libre para una búsqueda difusa por título/descripción, o filtra con tokens — priority:P0 label:\"Bug Fix\" agent:Codex status:review space:\"Client X\" is:attention is:home is:hidden has:port. Los chips de prioridad P0–P4, los chips de etiquetas y el embudo editan estos mismos tokens, así que escribir y hacer clic nunca se contradicen. La × lo borra todo. Gestiona las etiquetas en Configuración del proyecto → Etiquetas.",
	...
	"help.filters.dsl.body":
		"Escribe para una búsqueda difusa por títulos y descripciones, o filtra con tokens: priority:P0 label:\"Bug Fix\" agent:Codex status:review space:\"Client X\" is:attention is:home is:hidden has:port. Entrecomilla los valores con espacios. Combina facetas (Y); repite una faceta para ampliar (O). Los chips P0–P4, los chips de etiquetas y el embudo editan estos mismos tokens. Gestiona las etiquetas en Configuración del proyecto → Etiquetas.",
	...
	"help.settings.advancedExperience.body":
		"Comportamiento en beta: vale la pena, pero no está terminado, así que cada entrada indica si viene activada y puedes desactivarla función por función. La primera es el reordenado de derecha a izquierda, que viene activado: los paneles de terminal pintan hebreo y árabe en orden de lectura en lugar de invertidos. Es una capa solo de visualización sobre un motor de terminal que aún no tiene soporte propio de derecha a izquierda, por lo que la selección con el ratón y el paso del cursor por los enlaces en esas líneas siguen el orden subyacente.",
	...
	"help.modal.taskDetail.body":
		"Edita todo sobre la tarea: título, descripción (el prompt del agente), etiquetas, prioridad y estado. La fila de backend de terminal cambia tmux/nativo para el próximo arranque de esta tarea y se bloquea mientras su terminal está activo.",
	...
	"help.viewer.images.body":
		"Capturas y renders que un agente compartió con `dev3 show-image`, las más nuevas primero. Recorre el historial, descarga una imagen (también con clic derecho), copia su ruta o revela el archivo original en disco. Un vídeo compartido con `dev3 show-video` se reproduce aquí mismo: reproducir, pausar, buscar y volumen están en el reproductor, y nunca arranca solo.",
	...
	"help.sidebar.showHidden.body":
		"Ocultar una tarea saca todo su grupo de variantes de Tareas activas y nada más: sigue trabajando, sigue notificando y permanece en el tablero. Este ojo está junto al embudo de filtros, aparece solo cuando hay algo oculto y devuelve esas filas, atenuadas y marcadas, hasta que lo pulses otra vez. Un punto rojo indica que una de ellas te necesita. El token is:hidden del embudo hace lo mismo desde el filtro.",
