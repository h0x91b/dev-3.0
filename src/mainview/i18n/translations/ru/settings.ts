const settings = {
	...
	"projectSettings.tabProjectDesc": "Настройки проекта — хранятся локально на этой машине, не передаются команде",
	"projectSettings.tabWorktreeDesc": "Переопределения для worktree — приоритетнее, чем Project Config, когда заданы",
	...
	"projectSettings.worktreeRepoDesc": "Коммитится в git как .dev3/config.json — доступно всей команде. Переопределяет Project Config для этого worktree.",
	"projectSettings.worktreeLocalDesc": "Локальные переопределения — не попадает в git (.dev3/config.local.json). Наивысший приоритет, никому не виден.",
	"projectSettings.worktreePriorityHint": "Конфиг worktree приоритетнее, чем Project Config. Если поле задано здесь, значение из Project Config игнорируется для этой задачи.",
