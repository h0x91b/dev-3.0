...
const COMMANDS: CommandHelp[] = [
	...
	{
		name: "import",
		summary: "Put an agent session that ran outside dev3 on the board.",
		subcommands: [],
		usage: "dev3 import [--session <id>] [--yes|-y]",
		details: [
			"Run it from the shell the session ran in — the working directory is what",
			"decides which project the session belongs to.",
			"",
			"With no arguments it lists this project's importable sessions: index, title,",
			"turns, branch, age, and the short session id to pass to --session.",
			"--session <id>   Import that session (a unique id prefix is enough). dev3 forks",
			"                 a fresh worktree from the session's own branch and re-hosts",
			"                 the conversation there with --resume.",
			"--yes, -y        Skip the confirmation prompt. Required when stdin is not a",
			"                 terminal; exit 10 if the prompt is answered no.",
			"",
			"A session can only be imported into the project that owns the directory it ran",
			"in — there is deliberately no --project override. When no project owns the",
			"directory, dev3 says so and points at Add Project (exit 17).",
		],
	},
	{
		name: "install-skills",
