...
- **Too many projects to scan → group them into Spaces.** Once the project list stops being scannable, a **Space** groups projects: a rail on the dashboard lists every space with its project count, and picking one filters the overview to it — it never navigates away, so nothing gains a second board. Membership is **many-to-many** (a shared service can sit in both \`infra\` and \`client-x\`), and a project that belongs to several renders under each of them, carrying a chip for its other spaces. Create one from **+ New space** at the bottom of the rail — the overview header offers the same button only while the rail is off screen (a narrow window, or your very first space, when there is no rail yet); add existing projects with the **+** on a space header (it searches, and can start a brand-new project already linked); change one project's memberships from the row's **Spaces…** action or Project Settings → Spaces. Rename and delete live in the **…** on the space header — deleting only removes the grouping, never a project. Projects in no space collect under a computed **Home** group; **Home is not a real space** — it is not stored, cannot be renamed, and never appears in a membership picker. **Space order is edited only in the rail:** drag a row, or hit **Reorder** beside the Spaces label to get ↑/↓ per row (the path that works by touch and keyboard, where drag does not). Projects reorder by drag within their space. Neither ever changes membership. With zero spaces the dashboard is exactly the screen it always was.
