...
9. **Get it shipped → PR from the task.** Push & create the PR straight from the task, enable auto-merge, and let the task auto-complete when the PR merges. Every PR dev3 opens ends with a deep link back to its task, so anyone reading it can jump straight in — switch that off under **Settings → Tasks & Board → Link pull requests back to the task** if a public PR shouldn't reveal it was made with dev3. An agent does the same in one command: \`dev3 pr create --title "..." --description "..." [--auto-merge]\` pushes the branch, opens the PR with that footer already appended and targets the base branch the task was created from — and refuses with exit 23 before pushing anything if \`gh\` is missing or logged out.
