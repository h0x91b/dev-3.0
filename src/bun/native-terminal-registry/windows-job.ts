/**
 * Windows Job Object ownership for the native-session registry (seq 1214).
 *
 * A token-named, kill-on-close Job Object is the registry's Windows ownership
 * boundary: the detached host self-enrols BEFORE spawning its shell, so the
 * shell and every descendant inherit containment at process creation (no
 * assignment race). Job membership is also the reused-PID defence on Windows —
 * a stale record's PID that is NOT in this session's job is not ours.
 *
 * This is a deliberate, self-contained re-implementation of the approach proven
 * in decision 146 (the detached-pty spike) under a DISTINCT job namespace, so
 * the registry never depends on the removable spike. bun:ffi is used only on
 * Windows and only inside this module.
 */
