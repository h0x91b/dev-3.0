...
function makeRequest(params: Record<string, unknown>): CliRequest {
	return { id: "req-1", method: "task.requestCancellation", params };
}
...
describe("task.requestCancellation", () => {
	it("errors when the task is already cancelled", async () => {
		setupTask(makeTask({ status: "cancelled" }));

		const resp = await handleRequest(makeRequest({ taskId: "task-abc12345", projectId: "proj-1" }));
		expect(resp.ok).toBe(false);
		expect(resp.error).toContain("already cancelled");
	});
	...
	it("pushes agentCancellationRequested and cancels the task on approval", async () => {
		const task = makeTask({ overview: "Agent overview" });
		setupTask(task);
		const pushFn = vi.fn();
		vi.mocked(getPushMessage).mockReturnValue(pushFn);
		const cancelledTask = { ...task, status: "cancelled" as const };
		vi.mocked(moveTask).mockResolvedValue(cancelledTask);
		...
		// Its OWN channel, not the completion one: the two dialogs must never be
		// interchangeable, because only this one throws the work away.
		expect(event).toBe("agentCancellationRequested");
		...
		expect(resp.data).toEqual({ approved: true, task: cancelledTask });
		expect(moveTask).toHaveBeenCalledWith({ taskId: task.id, projectId: "proj-1", newStatus: "cancelled" });
	});
	...
	// Cancellation is its own request KIND, so a pending cancel must not be
	// answerable through the completion pending list (and vice versa) — that
	// would let a "yes" meant for one dialog destroy a worktree via the other.
	it("is replayable under the cancel kind and invisible to the complete kind", async () => {
		setupTask(makeTask());
		const pushFn = vi.fn();
		vi.mocked(getPushMessage).mockReturnValue(pushFn);

		const blocked = handleRequest(makeRequest({ taskId: "task-abc12345", projectId: "proj-1" }));
		await vi.waitFor(() => expect(pushFn).toHaveBeenCalledTimes(1));

		expect(listPendingAgentRequests("complete")).toHaveLength(0);
		const pending = listPendingAgentRequests("cancel");
