...
describe("handlers.getBranchStatus", () => {
	...
	it("returns zeros when task has no worktree", async () => {
		...
		expect(result).toEqual({ ahead: 0, behind: 0, canRebase: false, insertions: 0, deletions: 0, unpushed: 0, mergedByContent: false, diffFiles: 0, diffInsertions: 0, diffDeletions: 0 });
	});
	...
	it("returns branch status with canRebase=true when behind", async () => {
		...
		vi.mocked(git.getBranchDiffStats).mockResolvedValue({ files: 4, insertions: 50, deletions: 20 });
	...
	it("sets canRebase=false when not behind", async () => {
		...
		vi.mocked(git.getBranchDiffStats).mockResolvedValue({ files: 0, insertions: 0, deletions: 0 });
