...
// Ensure sound-bundled.ts exists (generated by scripts/generate-sound-bundled.ts,
// but in a fresh worktree it won't be there yet).
const soundBundledPath = `${import.meta.dir}/../src/bun/sound-bundled.ts`;
if (!(await Bun.file(soundBundledPath).exists())) {
	const gen = Bun.spawnSync(
		["bun", `${import.meta.dir}/generate-sound-bundled.ts`],
		{ stdout: "inherit", stderr: "inherit", env: process.env },
	);
	if (gen.exitCode !== 0) {
		console.error("Failed to generate sound-bundled.ts");
		process.exit(1);
	}
}
